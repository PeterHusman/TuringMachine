    ...
    partial class Program
    {
        static TransitionRuleTableTuringMachine<string, char> Parse(string s, char[] startingTapeValues)
        {
            ...
            for (int i = 0; i < table.Length; i++)
            {
                ...
                string[] items = action.Split(new char[] { '{', '}', ',', ':' }, StringSplitOptions.RemoveEmptyEntries).Select(a => a.Trim()).Select(a => a.StartsWith('\'') ? ("" + a[1]) : a).ToArray();
